...
class WebhookUpdateIssueCommentAgent implements BaseAgent<Context<"issue_comment">, void>{
    ...
    async handleEvent(event: Context<"issue_comment">): Promise<void> {
        ...
        try{
            ...
            await gitmateai.issueChunks.deleteCommentByOwnerRepoIssueCommentId(
                {
                    owner: owner,
                    repo: repo,
            ...
            logger.info(`Chunks for issue ${owner}/${repo}/${issue_number} of type ${IssueContentType.Comment} are deleted successfully!`);
        }catch (error){
